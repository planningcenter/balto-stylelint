...
Balto Stylelint is a GitHub Action (TypeScript, Node 20) that runs Stylelint only on changed files in a pull request and annotates only changed lines. Built with `@vercel/ncc` to bundle into a single `dist/index.js`.
...
src/
  ...
  stylelint_result.ts    Stylelint output parsing and annotation logic
  git_utils.ts           Git diff utilities (changed files/lines)
test/
  snapshots/             Canonical test fixture files
  v16/                   Stylelint v16 test workspace
